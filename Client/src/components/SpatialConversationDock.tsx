...
import type { ConversationZone, Player, RoomIdentity } from "../../../Shared/realtime";
...
const VideoTile = ({ stream, label, muted = false, volume = 1, compact = false }: VideoTileProps) => {
...
const ControlButton = ({ active = true, danger = false, label, onClick, children }: ControlButtonProps) => (
  <button
    type="button"
    onClick={onClick}
    aria-label={label}
...
interface SpatialConversationDockProps {
  ...
  mediaError: string;
  mediaStatus: string;
  prepareMedia: () => Promise<MediaStream>;
  toggleMicrophone: () => void;
  toggleCamera: () => void;
  setDeafened: (deafened: boolean) => void;
  setZoneLocked: (locked: boolean) => void;
